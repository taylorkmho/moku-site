<template>
  <div>
    ...
    <div class="resources-list-container">
      <article class="resources-list" v-for="(category, index) in categories" v-bind:key="index">
        <div
          class="resources-list__header-image"
          :style="{backgroundImage: `url(${category.items[0].assetUrl})`}"
        >
        ...
        <ul class="resources-list__list">
          ...
          >
            <a
              data-item-id="id"
              :href="item.fullUrl"
              class="resources-list-item__link"
              v-html="item.title"
            />
          </li>
        </ul>
